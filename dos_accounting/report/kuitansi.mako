...
<body>
	%for o in objects :
			...
			<table cellspacing="0" cellpadding="1px" width="100%" class="font12px" border="0">
		 		...
		 		<tr>
		 			<td class="font10px"><b>PV / ${o.number or ''}</b></td>
			...
			<table width="100%" class="font12px one" cellpadding="2px" border="0">
				...
				<tr>
					<td >TERBILANG</td>
					<td width="2%">:</td>
					<td style="border-bottom: dotted 1px black">${o.amount_string|title}</td>
				...
				<tr>
					...
					<td width="2%">:</td>
					<td style="border-bottom: dotted 1px black">Pembayaran Invoice No. ${o.name or ""}</td>
			...
			<table cellpadding="2px"  width="100%" class="font12px one" border="0">
				<tr>
					...
					%if o.amount_total > 0
					<td width="30%" rowspan="2" class="hmid" style="border:1px solid black; padding:5px; font-size:18px;">${o.company_id.currency_id.symbol} ${formatLang(o.amount_total) or formatLang(0)}</td>
					%else
					<td width="30%" rowspan="2" class="hmid" style="border:1px solid black; padding:5px; font-size:18px;">${o.company_id.currency_id.symbol} ${formatLang(o.amount_total) or formatLang(0)}</td>
					%endif
					<td  rowspan="2" width="30%"></td>
					%if o.state == 'draft':
					<td width="30%" class="hmid" >Jakarta, </td>
					%else :
					<td width="30%" class="hmid" >Jakarta, ${time.strftime('%d %B %Y', time.strptime( o.date_invoice,'%Y-%m-%d'))}</td>
					%endif
				</tr>
				<tr>
