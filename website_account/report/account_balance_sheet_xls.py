...
class balance_sheet_xls(orm.TransientModel):
    _name = 'balance.sheet.xls'
...
class account_balance_sheet_xls(report_xls):
    column_sizes = []
    ...
    def generate_xls_report_stn(self, _p, _xs, data, objects, wb):  
        ...
        row_pos = 0
        self.column_sizes = []
	records = self.pool.get('account.account').get_lines_balance_sheet(self.cr, self.uid, data)
	max_level = self.pool.get('account.account').max_level_bs + 1 -3 # Index starts with 0 but levels starts with 1, -3 to avoid 3 empty Rows
        ...
        for current_account in records:
            ...
            if current_account['type']=='report' or current_account['parent_id']==1 or current_account['parent_name']== data['chart_account_name']:
                continue
	    ...
	    """-3 to avoid 3 empty Rows"""
            c_specs = [
                ('account', 1, 0, 'text', current_account['name'], None, None, None, (current_account['level']>0 and current_account['level'] -3 or 0), max_level),                
            ]              
            if current_account['has_childs']:
                ...
                if current_account['name'][:5] != 'Total': # or current_account['name'][:5] == 'Total' """ Give this condition also if any issue in Formulas or to avoid Formulas"""
                    ...
                    if formula_dict.has_key(current_account['id']-0.5):
                        #print 'pppppppppppppppppppppppp', formula_dict, current_account['id'] , current_account['parent_name'], current_account['name']
            ...
            if formula_dict.has_key(current_account['parent_id']):
                formula_dict[current_account['parent_id']] += (current_account['user_type'] and (current_account['user_type'].lower().strip()=='expense' and ' - ' or ' + ') or ' + ') + t_cell
            else:
                formula_dict[current_account['parent_id']] = 'Round('+(current_account['user_type'] and (current_account['user_type'].lower().strip()=='expense' and ' - ' or '') or '') + t_cell              
                
            #print 'ccccccccccccccccccccccc', current_account['parent_id'], formula_dict      
    ...
    def generate_xls_report_prev(self, _p, _xs, data, objects, wb):
        ...
        self.column_sizes = []
	records = self.pool.get('account.account').get_lines_balance_sheet(self.cr, self.uid, data)
	max_level = self.pool.get('account.account').max_level_bs + 4 -3 # For Index 0 (+1), Index starts with 0 but levels starts with 1, -3 to avoid 3 empty Rows
        # set print header/footer
        ws.header_str = data['title']
        ws.footer_str = self.xls_footers['standard']
        # Title
        ...
        curr_lbl = 'Year '+ (datetime.strptime(from_date, '%Y-%m-%d')).strftime('%Y')        
	...
	style_percent.font.colour_index = xlwt.Style.colour_map['dark_blue_ega']       
	    ...
	    """ -2 To avoid 2 empty rows"""
            c_specs = [
                ('account', 1, 0, 'text', current_account['name'], None, None, None, (current_account['level']>0 and current_account['level'] -3  or 0), max_level-3),                
            ]              
...
account_balance_sheet_xls('report.account.account_report_balance_sheet_xls',
                  'account.account')
