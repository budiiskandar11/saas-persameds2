function openerp_account_tb(instance) {
...
module.trailbalanceWidget = instance.web.Widget.extend({
        template: 'trailbalanceWidget',
        events: {
        ...
        start: function(){
            window.document.title = 'Trail Balance - Odoo';   
            this._super();
            var self = this;
            this.title="Trail Balance";
		...
		if (daterange.trim()!='')
		{
        	...
        	return self.model_account
                    .call("print_report_pdf_tb", ({'data':{'form': {'chart_account_id': chart_account_id, 'display_account': display_account_id, 'fiscalyear_id': 1, 'date_from': from_dt, 'period_to': false, 'journal_ids': [], 'used_context': {'chart_account_id': chart_account_id, 'fiscalyear': 1, 'date_from': from_dt, 'date_to': to_dt, 'state': target_move, 'journal_ids': false}, 'date_to': to_dt, 'filter': 'filter_date', 'target_move': target_move, 'filter_string':date_str, 'title':title, 'company':company, 'filter_str':filter_str, 'from_print':true}}}))
		...
		if (daterange.trim()!='')
		{
		    ...
		    to_dt = date_to.toLocaleFormat('%m-%d-%Y');
                ...
                self.model_account = new instance.web.Model("trail.balance.xls");
        	return self.model_account
                    .call("xls_export", ({'data':{'form': {'chart_account_id': chart_account_id, 'display_account': display_account_id, 'fiscalyear_id': 1, 'date_from': from_dt, 'period_to': false, 'journal_ids': [], 'used_context': {'chart_account_id': chart_account_id, 'fiscalyear': 1, 'date_from': from_dt, 'date_to': to_dt, 'state': target_move, 'journal_ids': false}, 'date_to': to_dt, 'filter': 'filter_date', 'target_move': target_move, 'filter_string':date_str, 'title':title, 'company':company, 'filter_str':filter_str, 'from_print':true}}}))                    
                    .then(function(action){
                                stopsplash(1000);
                                return self.do_action(action);
                           });
        ...
        self.model_account = new instance.web.Model("account.account");
        self.model_account
                    .call("get_lines_trail_balance", ({'data':{'form': {'chart_account_id': chart_account_id, 'display_account': display_account_id, 'fiscalyear_id': 1, 'date_from': from_dt, 'period_to': false, 'journal_ids': [], 'used_context': {'chart_account_id': chart_account_id, 'fiscalyear': 1, 'date_from': from_dt, 'date_to': to_dt, 'state': target_move, 'journal_ids': false}, 'date_to': to_dt, 'filter': 'filter_date', 'target_move': target_move, 'from_load':true}}}))
                    .then(function(results) {
                        _(results).each(function (item) {
                        {//if(item['type']!='report' && item['parent_name'] !=chart_account_name)
                        {
                        var content = '<ul><li id='+item['id']+'><a><table id='+item['id']+'_'+item['has_childs']+' class="datatbl" ><tr><td valign=top STYLE="text-decoration:none;">'+(item['name'].slice(0,5)=='Total' ? '<b style="color:RoyalBlue">'+item['name']+'</b></td><td valign=top style="width:150px;text-align:right;padding-right: 0px;border-top: 1px solid black;border-bottom: 1px solid black;"><b style="color:RoyalBlue">'+(item['debit']).toFixed(2)+'</b></td><td valign=top style="width:150px;text-align:right;padding-right: 0px;border-top: 1px solid black;border-bottom: 1px solid black;"><b style="color:RoyalBlue">'+(item['credit']).toFixed(2)+'</b></td><td valign=top style="width:150px;text-align:right;padding-right: 10px;border-top: 1px solid black;border-bottom: 1px solid black;">'+(item['balance']>=0 ? '<b style="color:RoyalBlue">'+(item['balance']).toFixed(2)+'</b>':'')+'</td><td valign=top style="width:150px;text-align:right;padding-right: 10px;border-top: 1px solid black;border-bottom: 1px solid black;">'+(item['balance']<=0 ? '<b style="color:RoyalBlue">'+(Math.abs(item['balance'])).toFixed(2)+'</b>':'')+'</td><!--td valign=top style="width:150px;text-align:right;padding-right: 10px;border-top: 1px solid black;border-bottom: 1px solid black;">'+(item['has_childs']==false ? '<b style="color:RoyalBlue">'+(item['balance']).toFixed(2)+'</b>':'')+'</td--></tr><tr><td height=25></td><td></td><td></td><td></td></tr></table></a></li></ul>':(item['has_childs']==true ? '<b  style="color:RoyalBlue">'+item['name']+'</b></td><td></td><td></td><td></td>':item['name']+'</td><td valign=top style="width:150px;text-align:right;">'+(item['debit']).toFixed(2)+'</td><td valign=top style="width:150px;text-align:right;">'+(item['credit']).toFixed(2)+'</td><td style="width:150px;text-align:right;padding-right: 10px;">'+(item['balance']>0 ? (item['balance']).toFixed(2):'0.00')+'</td><td style="width:150px;text-align:right;padding-right: 10px;">'+(item['balance']<0 ? (Math.abs(item['balance'])).toFixed(2):'0.00')+'</td><!--td style="width:150px;text-align:right;padding-right: 10px;">'+(item['balance']).toFixed(2)+'</td-->')+'</tr></table></a></li></ul>')
    ...
    openerp.web.client_actions.add('website_account.trailbalance', 'instance.website_account.trailbalanceWidget');
}
