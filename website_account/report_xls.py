...
class report_xls(report_sxw):
    ...
    xls_styles = {
        ...
        'fill_yellow': 'pattern: pattern solid, fore_color 0x0D;',
        'borders_all':
            'borders: '
    ...
    def xls_row_template(self, specs, wanted_list):
        ...
        - 'specs': list with Column Characteristics
            ...
            6: Column Style
            7: Formula
            8: User Defined Column Number
            9: Next Cell to Print
            10: Bold the Row
        """
        r = []
        col = 0
            ...
            for s in specs:
                if s[0] == w:
                    ...
                    if s_len > 7 and s[7] is not None:
                        ...
                        c.append(None)
                    if s_len > 8 and s[8] is not None:
                        col = s[8]
                    r.append((col, c[1], c))
                    if s_len > 9 and s[9] is not None:
                        col = (s[9] - 1) + c[1]
                    else:
                        col += c[1]
                    if s_len > 10 and s[10] is not None:
                        c.append(s[10])
                    else:
                        c.append(None)    
                    break
    ...
    def xls_write_row(self, ws, row_pos, row_data,
                      row_style=default_style, set_column_size=False, set_row_height=False):
        r = ws.row(row_pos)
