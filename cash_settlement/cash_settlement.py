...
#    _rec_name = 'number'
    _columns = {
        ...
        'name':fields.char('Memo', size=256, readonly=True,),
        ...
        'line_ids':fields.one2many('cash.settlement.line','voucher_id','Voucher Lines', readonly=True, states={'draft':[('readonly',False)]}),
...
#        'currency_id': fields.related('journal_id','currency', type='many2one', relation='res.currency', string='Currency', store=True, readonly=True, states={'draft':[('readonly',False)]}),
        ...
        'date_req': fields.date("Advance Request Date", readonly=True),
    ...
    def action_move_line_create2(self, cr, uid, ids, context=None):
        ...
        for inv in self.browse(cr, uid, ids, context=context):
            ...
            move_line = {
                ...
                'amount_currency': company_currency <> current_currency and sign * inv.reserved or 0.0,
                'date': inv.date,
            ...
            sign_diff = xdebit - xcredit < 0 and -1 or 1
            ...
            move_line2 = {
                ...
                'partner_id': inv.partner_id.id,
                'currency_id': company_currency <> current_currency and  current_currency or False,
                'amount_currency': company_currency <> current_currency and sign_diff * inv.settlement_amount or 0.0,
                'date': inv.date,
                'date_maturity': inv.date_due
            }
            if move_line2['debit'] <> 0.0 and move_line2['debit'] <> 0.0: 
            ...
            if not currency_pool.is_zero(cr, uid, inv_currency_id, line_total):
                print "Line Total",line_total , "balance_diff", balance_diff 
                balance_diff = currency_pool.compute(cr, uid, current_currency, company_currency, balance_diff, context=context_multi_currency)
...
class cash_settlement_line(osv.osv):
    ...
    def onchange_expense_id(self, cr, uid, ids, expense_type):
        account_id={}
        
        for type in self.pool.get('expense.type').browse(cr, uid, [expense_type], context=None):
