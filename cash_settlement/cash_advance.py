...
class cash_advance(osv.osv):
    ...
    def _get_currency_base(self, cr, uid, context=None):
        ...
        cur_comp = self.pool.get('res.company').browse
...
#        return [(r['id'], (str("%.2f" % r['amount']) or '')) for r in self.read(cr, uid, ids, ['amount'], context, load='_classic_write')]
    ...
    def onchange_employee (self, cr, uid, ids, employee_id):
        ...
        try:
            ...
            for onchange in self.pool.get('hr.employee').browse(cr, uid, employee_search):
                department = onchange.department_id.id
    ...
    def _compute_total_line(self, cr, uid, ids, field_name, args, context=None):
        rs_data = {}
        cur_obj = self.pool.get('res.currency')
        amount = 0.0
        res = {}
        for order in self.browse(cr, uid, ids, context=context): 
            res[order.id] = {'amount' : 0.0}
            ...
            res[order.id]['amount'] = cur_obj.round(cr, uid, cur, amount)
...
#    _rec_name = 'number'
    _columns = {
        ...
        'journal_id': fields.many2one('account.journal', 'Bank/ Cash Payment', required=True, readonly=True, states={'draft': [('readonly', False)]}),
...
#        'currency_id': fields.related('journal_id','currency', type='many2one', relation='res.currency', string='Currency', store=True, readonly=True, states={'draft': [('readonly', False)]}),
        ...
        'account_advance_id':fields.many2one('account.account', 'Advance Account', required=False),
    ...
    def onchange_type_id(self, cr, uid, ids, advance_type):
        account_advance_id={}
        
        for type in self.pool.get('cash.advance.type').browse(cr, uid, [advance_type], context=None):
        
            print "dkdkdkdkdkkdkdkdkd"
            account_advance_id = type.account_advance_id
    ...
    def propose(self, cr, uid, ids, context=None):
        return self.write(cr, uid, ids, {'state': 'confirm',
                                         'number': self.pool.get('ir.sequence').get(cr, uid, 'cash.advance')}
                                         , context=context)
...
#                        raise osv.except_osv(_('No Account Base Code and Account Tax Code!'),_("You have to configure account base code and account tax code on the '%s' tax!") % (tax_data.name))
            ...
            move_line['amount_currency'] = company_currency <> current_currency and sign * amount_currency or 0.0
            #DI Geser#
...
class cash_advance_line(osv.osv):
    ...
    _columns = {
        ...
        'expense_type' : fields.many2one('expense.type','Expense Group'),
    }
    _defaults = {
        'name': ''
    }
...
class cash_advance_type(osv.osv):
    ...
    _columns    = {
                   ...
                   'account_advance_id' : fields.many2one('account.account', 'Account Advance', required=False)
