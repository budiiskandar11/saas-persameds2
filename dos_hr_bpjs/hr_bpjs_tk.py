...
class hr_contract(osv.osv):
    ...
    def _calculate_salary(self, cr, uid, ids, field_names, arg, context=None):
        ...
        for contract in self.browse(cr, uid, ids, context=context):
            ###Uang Makan###
            uang_makan      = 22 * contract.uang_makan
            ###Uang Transport###
            uang_transport  = 22 * contract.uang_transport
...
class hr_bpjs_tk(osv.osv):
    ...
    def _calculate(self, cr, uid, ids, field_names, arg, context=None):
        ...
        for rs in self.browse(cr, uid, ids, context=context):
            ...
            jht_by_employee     = 0.0
            jht_by_company      = 0.0
            jk_pensiun_employee = 0.0
            jk_pensiun_company  = 0.0
            jpk_amount          = 0.0
            jkk_amount          = 0.0
            jk_amount           = 0.0
            if rs.jht:
            ...
            if rs.jk_pensiun:
                if rs.contract_wage > 7000000: 
                    thp = 7000000
                else:
                    thp = rs.contract_wage
                jk_pensiun_employee = thp*0.01
                jk_pensiun_company  = thp*0.02
            if rs.jkk:
                jkk_amount=(rs.contract_wage)*0.0024
            ...
            record = {
                      ...
                      'jk_pensiun_employee' : jk_pensiun_employee, 
                      'jk_pensiun_company'  : jk_pensiun_company,
                      ...
                      'total_by_employee' : jht_by_employee + jk_pensiun_employee,
                      'total_by_company' : jht_by_company + jk_pensiun_company + jk_amount + jkk_amount,
    ...
    _columns = {
                ...
                'jk_pensiun_employee'  : fields.function(_calculate, method=True, store=False, multi='dc', string='Dana Pensiun by Employee', digits_compute=dp.get_precision('Account')),
                'jk_pensiun_company'  : fields.function(_calculate, method=True, store=False, multi='dc', string='Dana Pensiun by Company', digits_compute=dp.get_precision('Account')),
